...
export class TemplatesController {
  ...
  @Post()
  create(@CurrentUser() user: User, @Body() body: { name: string; category?: string; body: string; variables?: string[] }) {
    return this.svc.create(user.companyId, body);
  }

  @Patch(':id')
  update(@CurrentUser() user: User, @Param('id') id: string, @Body() body: { name?: string; category?: string; body?: string; variables?: string[] }) {
