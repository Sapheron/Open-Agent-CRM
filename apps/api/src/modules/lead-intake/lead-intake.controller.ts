...
export class LeadIntakeController {
  constructor(private readonly svc: LeadIntakeService) {}
  ...
  eligibility() {
    ...
    return {
      ...result,
      customWebhookUrl: buildCustomWebhookUrl(),
    };
  }
  ...
  async list(@CurrentUser() user: User) {
    ...
    return items.map((i) => ({
      ...i,
      webhookUrl: i.provider === 'META_ADS' ? buildMetaWebhookUrl(i.id) : null,
    }));
  }
  ...
  async get(@CurrentUser() user: User, @Param('id') id: string) {
    ...
    return {
      ...integration,
      webhookUrl: integration.provider === 'META_ADS' ? buildMetaWebhookUrl(integration.id) : null,
    };
  }
